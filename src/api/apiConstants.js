...
const makeData = (allFilter , page , url=productBaseUrl) =>{
  let queryParam = {}
  ...
  if(page){
    for(let i in page[0]){
      str.searchParams.append(i , page[0][i])
    }
  }
  return str
}

export const FetchTableData = async(allFilter , page ) =>{
  let str = makeData(allFilter , page)
